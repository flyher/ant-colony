// fix from https://github.com/leo108/jsonFormater
// let imgCollapsed = require('./images/collapsed.png');
// let imgExpanded = require('./images/expanded.png');
...
JsonFormater.prototype = {
  ...
  doFormat: function (json) {
    ...
    try {
      ...
      return '<pre class="row" style="margin:0 0 0 0">' + html + '</pre>';
    } catch (e) {
      return '';
      this.isFormated = false;
    }
  ...
  makeContentVisible: function (element, visible) {
    ...
    var btn = element.prev().find('.imgToggle');
    if (visible) {
      element.show();
  ...
  ProcessObject: function (obj, indent, addComma, isArray, isPropertyContent) {
    ...
    if (this.isArray(obj)) {
      if (obj.length === 0) {
        ...
        clpsHtml = this.options.isCollapsible ? "<span><span class='imgToggle' data-status='1'/>" + this.options.imgExpanded + "</span></span><span class='jf-collapsible'>" : "";
